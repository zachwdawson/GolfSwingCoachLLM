...
# Data source for VPC (needed for target groups)
...
resource "aws_lb_listener_rule" "api" {
  ...
  condition {
    path_pattern {
      values = ["/api/*", "/upload", "/videos/*", "/health", "/ready"]
    }
  }
}
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        {
          name  = "DB_URL"
          value = "postgresql://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.endpoint}/golfcoach"
        },
        {
          name  = "AWS_REGION"
      ...
      secrets = var.aws_access_key_id_secret_arn != "" ? [
        {
          name      = "AWS_ACCESS_KEY_ID"
          valueFrom = var.aws_access_key_id_secret_arn
        },
        {
          name      = "AWS_SECRET_ACCESS_KEY"
          valueFrom = var.aws_secret_access_key_secret_arn
        },
        {
          name      = "OPENAI_API_KEY"
